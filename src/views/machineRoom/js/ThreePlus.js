...
export default class ThreePlus{
    ...
    setHemisphereLight(color, intensity){
        const hemisphere = new THREE.HemisphereLight(0x75D6FF, 0x00040A,intensity);
    ...
    setSunLight(name,position,color,intensity){
        ...
        if(name == 'sunLight' && shadowShow){
            ...
            this.shadowBox(sunlight,-50,50,-50,50,0.1,50);
    ...
    addMachine(){
        let machineRoom = new machine(this.scene, this.camera);
        this.machine = machineRoom;
        return machineRoom;
    }

    addReflectorPlane(){
        eventHub.on("addReflector", (item)=>{
            console.log(item)
            this.groundReflector = item;
            this.addSSRPass()
        })
    }
