...
export default {
  ...
  methods: {
    ...
    updateScale() {
      ...
      this.$store.state.scaleX = scale;
      this.$store.state.scaleY = scale;
    ...
    async resize() {
      ...
      // 视频全屏时不调整
      if ((document.mozFullScreen ||document.webkitIsFullScreen ||document.webkitFullScreen) && !this.$store.state.isFullScreen) {
        return
      }
      await this.initSize();
      this.updateSize();
